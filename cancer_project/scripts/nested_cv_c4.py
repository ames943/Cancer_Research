#!/usr/bin/env python3
"""
Nested LOOCV for C4 (Lee 2022, n=165) alone — no batch correction (single cohort).

Same hyperparameter grids + timing-probe logic as nested_cv.py (Phase 3a).
Outputs: results/ml/lee2022/nested_cv_{results,summary,best_params,perm}.tsv

Run from cancer_project/:
    nohup python3 scripts/nested_cv_c4.py > logs/nested_cv_c4.log 2>&1 &
"""
...
print(f"[{time.strftime('%H:%M:%S')}] Loading C4 data …", flush=True)
...
# ── per-fold helpers (no batch correction for single cohort) ──────────────────

def feature_select_df(clr_tr: pd.DataFrame, raw_tr: pd.DataFrame,
                      resp_tr: pd.Series) -> list:
    n          = len(clr_tr)
...
def inner_cv_select(tr_clr, tr_raw, tr_response, en_grid, rf_grid, xgb_grid, fold_seed):
    ...
    for itr_i, iva_i in skf.split(tr_clr.values, y_bin_full):
        itr_clr  = tr_clr.iloc[itr_i]
        iva_clr  = tr_clr.iloc[iva_i]
        itr_raw  = tr_raw.iloc[itr_i]
        itr_resp = tr_response.iloc[itr_i]
        iva_resp = tr_response.iloc[iva_i]

        isel = feature_select_df(itr_clr, itr_raw, itr_resp)
        iX_tr = itr_clr[isel].values
        iX_va = iva_clr[isel].values
        ...
        for gi, params in enumerate(en_grid):
            try:
                ...
                en_fold_aucs[gi].append(
                    roc_auc_score(iy_bin_va, m.predict_proba(iX_va)[:, classes.index("R")])
                )
            except Exception:
                pass
        ...
        for gi, params in enumerate(xgb_grid):
            try:
                ...
                xgb_fold_aucs[gi].append(
                    roc_auc_score(iy_bin_va, m.predict_proba(iX_va)[:, 1])
                )
            except Exception:
                pass
...
def run_one_outer_fold(held_out_idx, resp_vec, xgb_grid):
    ...
    sel   = feature_select_df(tr_clr, tr_raw, tr_resp)
    ...
    best_en, best_rf, best_xgb, auc_en, auc_rf, auc_xgb = inner_cv_select(
        tr_clr, tr_raw, tr_resp, EN_GRID, RF_GRID, xgb_grid, fold_seed,
    )
    ...
    best_params = {"ElasticNet": best_en, "RandomForest": best_rf, "XGBoost": best_xgb}
...
def run_one_outer_fold_fixed(held_out_idx, resp_vec, params_en, params_rf, params_xgb):
    ...
    sel   = feature_select_df(tr_clr, tr_raw, tr_resp)
    X_tr  = tr_clr[sel].values; y_tr = tr_resp.values; X_te = te_clr[sel].values

    m_en = LogisticRegression(
        penalty="elasticnet", solver="saga", C=params_en["C"], l1_ratio=params_en["l1_ratio"],
        class_weight="balanced", max_iter=5000, tol=1e-3, random_state=SEED,
    )
    m_en.fit(X_tr, y_tr)
    classes_en = list(m_en.classes_)
...
t_proj = t_per_fold * N
print(f"  {t_per_fold:.1f}s/fold → projected total: {t_proj/3600:.2f}h", flush=True)
...
if t_proj / 3600 > MAX_RUNTIME_HOURS:
    ...
    print(f"  [OK] Using full XGB grid ({len(XGB_GRID_FULL)} configs)", flush=True)
...
for fold_num, held_out in enumerate(SAMPLES, start=1):
    ...
    if fold_num % 20 == 0 or fold_num == 1 or fold_num == N:
...
print(f"\n{'='*65}", flush=True)
print("NESTED LOOCV — C4 (Lee 2022, n=165), single-cohort, no batch correction", flush=True)
...
results_df.to_csv(f"{OUT_DIR}/nested_cv_c4_results.tsv", sep="\t", index=False)
summary_rows = [{"model": mn, **metrics[mn]} for mn in ["ElasticNet","RandomForest","XGBoost"]]
pd.DataFrame(summary_rows).to_csv(f"{OUT_DIR}/nested_cv_c4_summary.tsv", sep="\t", index=False)
...
pd.DataFrame(perm_summary).to_csv(f"{OUT_DIR}/nested_cv_c4_perm_summary.tsv", sep="\t", index=False)
perm_auc_df = pd.DataFrame(perm_aucs)
perm_auc_df.to_csv(f"{OUT_DIR}/nested_cv_c4_perm_aucs.tsv", sep="\t", index=False)

print(f"\n  Saved: {OUT_DIR}/nested_cv_c4_results.tsv", flush=True)
print(f"  Saved: {OUT_DIR}/nested_cv_c4_summary.tsv", flush=True)
print(f"  Saved: {OUT_DIR}/nested_cv_c4_perm_summary.tsv", flush=True)
print(f"\n[{time.strftime('%H:%M:%S')}] DONE — C4 nested CV complete", flush=True)
